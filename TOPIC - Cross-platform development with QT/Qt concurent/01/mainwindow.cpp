...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        ui->te_debug->append("Запуск последовательных задач с помощью QtConcurrent");
        //here homework
        QFuture future1 = QtConcurrent::run(&ExampleRace::DoWork, concurRace1, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
        future1.waitForFinished();
        QFuture future2 = QtConcurrent::run(&ExampleRace::DoWork, concurRace2, &number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value());
    }
    else{
